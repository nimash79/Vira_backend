...
const {
  ...
  userMiddleWare,
  hardwareMiddleWare,
} = require("./middlewares/api_middleware");
const { createBinary } = require("./controllers/deviceController");
...
app.get("/download", async (req, res) => {
  const device = await Device.findOne({ deviceId: "12345" });

  const buffer = createBinary(device);

  res.set({
    "Content-Type": "application/octet-stream",
    "Content-Disposition": 'attachment; filename="jahesh.txt"',
